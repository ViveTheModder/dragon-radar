...
public class CsvHandler {
	...
	public static File[] getAvailableCsvFiles() {
		File csvFolder = new File("./res/csv/");
	...
	public static int getCsvSearchResult(File[] csvFiles, String type) {
		File csvKey = new File("./res/csv/" + type + ".csv");
		int csvIndex = Arrays.binarySearch(csvFiles, csvKey);
		return csvIndex;
	}
	public static String[] getLines(File csv) throws IOException {
		int lineCnt = 0;
		int numParamNames = getNumRows(csv) - 1;
		...
		if (sc.hasNextLine()) {
			...
			if (!header.equals("pos,name,tooltip")) {
				sc.close();
				return null;
			}
		}
		...
		while (sc.hasNextLine()) {
			...
			if (lineCnt < numParamNames) paramNames[lineCnt] = line;
			lineCnt++;
		}
		sc.close();
		return paramNames;
